...
[palindrome-app](http://www.palindrome-app.org/) is an open source identity and access management for modern applications and services.

## TL;DR;

```console
$ helm install palindrome-app codecentric/palindrome-app
```

## Introduction

This chart bootstraps a [palindrome-app](http://www.palindrome-app.org/) StatefulSet on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
It provisions a fully featured palindrome-app installation.
For more information on palindrome-app and its capabilities, see its [documentation](http://www.palindrome-app.org/documentation.html).
...
To install the chart with the release name `palindrome-app`:

```console
In current directory (OBP-Product-App-CM-Portal-Backend/obp-cms-backend/helm/palindrome-app)
$ helm upgrade --install palindrome-app . --namespace obp-cms-portal --create-namespace --values values.yaml

Can access the admin console in localhost at http://localhost:5080/auth
```

## Uninstalling the Chart

To uninstall the `palindrome-app` deployment:

```console
$ helm uninstall palindrome-app -n obp-cms-portal
```

## Configuration

The following table lists the configurable parameters of the palindrome-app chart and their default values.
...
| `image.repository` | The palindrome-app image repository                                                                                                                                                                                                                                                     | `quay.io/palindrome-app/palindrome-app` |
| `image.tag` | Overrides the palindrome-app image tag whose default is the chart version. Note versions after 16.1.1 need to use the `-legacy` suffix, e.g. `17.0.1-legacy`                                                                                                                              | `""` |
| `image.pullPolicy` | The palindrome-app image pull policy                                                                                                                                                                                                                                                    | `IfNotPresent` |
...
| `securityContext` | SecurityContext for the palindrome-app container                                                                                                                                                                                                                                        | `{"runAsNonRoot":true,"runAsUser":1000}` |
...
| `lifecycleHooks` | Lifecycle hooks for the palindrome-app container                                                                                                                                                                                                                                        | `{}` |
| `terminationGracePeriodSeconds` | Termination grace period in seconds for palindrome-app shutdown. Clusters with a large cache might need to extend this to give Infinispan more time to rebalance                                                                                                                        | `60` |
| `clusterDomain` | The internal Kubernetes cluster domain                                                                                                                                                                                                                                            | `cluster.local` |
| `command` | Overrides the default entrypoint of the palindrome-app container                                                                                                                                                                                                                        | `[]` |
| `args` | Overrides the default args for the palindrome-app container                                                                                                                                                                                                                             | `[]` |
| `extraEnv` | Additional environment variables for palindrome-app                                                                                                                                                                                                                                     | `""` |
| `extraEnvFrom` | Additional environment variables for palindrome-app mapped from a Secret or ConfigMap                                                                                                                                                                                                   | `""` |
...
| `startupScripts` | Startup scripts to run before palindrome-app starts up                                                                                                                                                                                                                                  | `{"palindrome-app.cli":"{{- .Files.Get "scripts/palindrome-app.cli" \| nindent 2 }}"}` |
...
| `ingress.rules[0].host` | Host for the Ingress rule                                                                                                                                                                                                                                                         | `{{ .Release.Name }}.palindrome-app.example.com` |
...
| `ingress.tls[0].hosts` | List of TLS hosts                                                                                                                                                                                                                                                                 | `[palindrome-app.example.com]` |
...
| `ingress.console.rules[0].host` | Host for the Ingress rule for the console                                                                                                                                                                                                                                         | `{{ .Release.Name }}.palindrome-app.example.com` |
...
| `ingress.console.tls[0].hosts` | List of TLS hosts                                                                                                                                                                                                                                                                 | `[palindrome-app.example.com]` |
...
| `postgresql.postgresqlUsername` | PostgreSQL User to create                                                                                                                                                                                                                                                         | `palindrome-app` |
| `postgresql.postgresqlPassword` | PostgreSQL Password for the new user                                                                                                                                                                                                                                              | `palindrome-app` |
| `postgresql.postgresqlDatabase` | PostgreSQL Database to create                                                                                                                                                                                                                                                     | `palindrome-app` |
...
| `extraServiceMonitor.enabled` | If `true`, an additional ServiceMonitor resource for the prometheus-operator is created. Could be used for additional metrics via [palindrome-app Metrics SPI](https://github.com/aerogear/palindrome-app-metrics-spi)                                                                        | `false` |
...
```console
$ helm install palindrome-app codecentric/palindrome-app -n palindrome-app --set replicas=1
```
...
```console
$ helm install palindrome-app codecentric/palindrome-app -n palindrome-app --values values.yaml
```

The chart offers great flexibility.
It can be configured to work with the official palindrome-app Docker image but any custom image can be used as well.

For the offical Docker image, please check it's configuration at https://github.com/palindrome-app/palindrome-app-containers/tree/master/server.
...
palindrome-app sets the following system properties by default:
...
The palindrome-app Docker image supports various database types.
...
extraEnvFrom: |
  - secretRef:
      name: '{{ include "palindrome-app.fullname" . }}-db'
...
extraVolumes: |
  - name: db-creds
    secret:
      secretName: palindrome-app-db-creds
```

### Creating a palindrome-app Admin User

The palindrome-app Docker image supports creating an initial admin user.
It must be configured via environment variables:

* `palindrome-app_USER` or `palindrome-app_USER_FILE`
* `palindrome-app_PASSWORD` or `palindrome-app_PASSWORD_FILE`

Please refer to the section on database configuration for how to configure a secret for this.

### High Availability and Clustering

For high availability, palindrome-app must be run with multiple replicas (`replicas > 1`).
The chart has a helper template (`palindrome-app.serviceDnsName`) that creates the DNS name based on the headless service.
...
Recent versions of palindrome-app include a new Kubernetes native [KUBE_PING](https://github.com/jgroups-extras/jgroups-kubernetes) service discovery protocol.
...
However, the palindrome-app Pods must also get RBAC permissions to `get` and `list` Pods in the namespace which can be configured as follows:
...
Due to the caches in palindrome-app only replicating to a few nodes (two in the example configuration above) and the limited controls around autoscaling built into Kubernetes, it has historically been problematic to autoscale palindrome-app.
...
### Running palindrome-app Behind a Reverse Proxy

When running palindrome-app behind a reverse proxy, which is the case when using an ingress controller,
...
One option is certainly to provide a custom palindrome-app image that includes the theme.
However, if you prefer to stick with the official palindrome-app image, you can use an init container as theme provider.
...
In combination with an `emptyDir` that is shared with the palindrome-app container, configure an init container that runs your theme image and copies the theme over to the right place where palindrome-app will pick it up automatically.
...
extraVolumeMounts: |
  - name: theme
    mountPath: /opt/jboss/palindrome-app/themes/mytheme
...
It can be mounted using `extraVolumeMounts` and then referenced as environment variable `palindrome-app_IMPORT`.
...
By default, palindrome-app is served under context `/auth`.
...
startupScripts:
  ...
  contextPath.cli: |
    ...
    /subsystem=palindrome-app-server/:write-attribute(name=web-context,value={{ if eq .Values.contextPath "" }}/{{ else }}{{ .Values.contextPath }}{{ end }})
    {{- if eq .Values.contextPath "" }}
    /subsystem=undertow/server=default-server/host=default-host:write-attribute(name=default-web-module,value=palindrome-app-server.war)
    {{- end }}
    {{- end }}
    run-batch
...
In order to achieve this, the environment variable `palindrome-app_STATISTICS` must be set.
...
#### palindrome-app Metrics SPI

Optionally, it is possible to add [palindrome-app Metrics SPI](https://github.com/aerogear/palindrome-app-metrics-spi) via init container.
...
* palindrome-app is updated to 17.0.1
* Use image from quay.io/palindrome-app/palindrome-app

Note that we now need to add the `-legacy` suffix for the `palindrome-app Legacy` image for the Wildfly based palindrome-app as documented in [this palindrome-app Blog Post](https://www.palindrome-app.org/2022/02/palindrome-app-1700-released#_migrating_from_the_preview_quarkus_distribution),
e.g.: `quay.io/palindrome-app/palindrome-app:17.0.1-legacy`.


Please read the additional notes about [Migrating to 17.0.0](https://www.palindrome-app.org/docs/latest/upgrading/index.html#migrating-to-17-0-0) in the palindrome-app documentation.

### From chart < 16.0.0

* palindrome-app is updated to 16.1.1

Please read the additional notes about [Migrating to 16.0.0](https://www.palindrome-app.org/docs/latest/upgrading/index.html#migrating-to-16-0-0) in the palindrome-app documentation.

### From chart < 15.0.0

* palindrome-app is updated to 15.0.2
...
* palindrome-app is updated to 14.0.0

Note that this might not be a seamless upgrade, because the clustering with older palindrome-app versions might not work
due to incompatible infinispan versions.

### From chart < 12.0.0

* palindrome-app is updated to 13.0.1
...
```
kubectl delete sts <RELEASE_NAME>-palindrome-app && helm upgrade --install
```
...
* palindrome-app is updated to 12.0.4
...
The palindrome-app chart received a major facelift and, thus, comes with breaking changes.
Opinionated stuff and things that are now baked into palindrome-app's Docker image were removed.
...
* Values are no longer nested under `palindrome-app`.
...
* palindrome-app is updated to 10.0.0
...
Version 7.0.0 update breaks backwards-compatibility with the existing `palindrome-app.persistence.existingSecret` scheme.
...
The variable referencing the password key name has been renamed from `palindrome-app.persistence.existingSecretKey` to `palindrome-app.persistence.existingSecretPasswordKey`

A new, optional variable for referencing the username key name for populating the `DB_USER` env has been added:
`palindrome-app.persistence.existingSecretUsernameKey`.

If `palindrome-app.persistence.existingSecret` is left unset, a new Secret will be provisioned populated with the `dbUser` and `dbPassword` Helm variables.

###### Example configuration:
```yaml
palindrome-app:
  persistence:
    existingSecret: palindrome-app-provisioned-db-credentials
...
```yaml
  livenessProbe: |
    httpGet:
      path: {{ if ne .Values.palindrome-app.basepath "" }}/{{ .Values.palindrome-app.basepath }}{{ end }}/
      port: http
    initialDelaySeconds: 300
    timeoutSeconds: 5
  readinessProbe: |
    httpGet:
      path: {{ if ne .Values.palindrome-app.basepath "" }}/{{ .Values.palindrome-app.basepath }}{{ end }}/realms/master
      port: http
    initialDelaySeconds: 30
    timeoutSeconds: 1
```
...
Applies to `palindrome-app.existingSecret` and `palindrome-app.persistence.existingSecret`.

_`values.yaml` of parent chart:_
```yaml
palindrome-app:
  palindrome-app:
    existingSecret: '{{ .Release.Name }}-palindrome-app-secret'
```
...
* The chart uses the new support for running scripts at startup that has been added to palindrome-app's Docker image.
...
kubectl delete statefulset -n "$namespace" -l app=palindrome-app -l release="$release" --cascade=false

kubectl label pod -n "$namespace" -l app=palindrome-app -l release="$release" app.kubernetes.io/name=palindrome-app
kubectl label pod -n "$namespace" -l app=palindrome-app -l release="$release" app.kubernetes.io/instance="$release"
```
